...
import { deliverOrchestrationPrompt } from "../engine/prompt.ts"
import { normalizeDependsOn, waitNodeIds } from "../engine/depends-on.ts"
...
import { waitForOrchestration } from "../engine/wait.ts"
...
export function createMissionContext(input: {
  ...
  function resolvePlan() {
    const plan = readLatestOrchestrationPlanRecord(plugin.directory, missionId)
    if (!plan) {
      throw new Error(`orchestration plan missing for ${missionId}; submit mission.script.ts first`)
    }
    return plan
  }
