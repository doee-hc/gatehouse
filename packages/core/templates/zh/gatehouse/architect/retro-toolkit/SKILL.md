...
1. **唯一数据源** — 你所辖分支的 `context/`（`messages.json`、`timeline.md`、`metrics.json`、`subtree-metrics.json`）。
...
4. **工具持久化** — 有效脚本 + 使用说明写入本目录 `tools/<verb-noun>/`（见下方模板），下次 retro 先读已有工具再决定是否扩展。

## 落盘 vs 自制（边界）

| 来源 | 用途 |
|------|------|
| `context/` 落盘 | 全量原始消息 + grep 友好 timeline — **特征提取的主原料** |
| `context/*/metrics.json` | 单节点 session 级 token/耗时/工具聚合（系统预计算） |
| `context/subtree-metrics.json` | 各 retro coord 所辖子树聚合（读 `retro_nodes[<你的 node_id>]`） |
