...
1. Call `gatehouse_mission_info` for mission snapshot and orchestration state (completed nodes / baseline).
2. Update `.gatehouse/trees/{{mission_id}}/mission.script.ts` to cover **only the revision** (skip `done` leaves; reference their output via `rollupFrom` in later steps).
