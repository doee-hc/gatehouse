...
name: lead-planning
description: >-
  外层任务负责人 (profile lead) — missions.yaml, acceptance, retro kickoff.
---
...
4. **下一项任务** — 读{{architect_name}} `architect-summary.md`（及{{curator_name}}摘要若有），结合用户评价规划。
...
模板：`planning-skill/missions.template.yaml`（若存在）或直接参照下方字段示例。
