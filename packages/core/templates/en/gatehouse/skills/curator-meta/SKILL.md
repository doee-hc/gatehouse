...
name: curator-meta
description: >-
  Assigns skill domains, rolls up domain skills, and iterates the global extract prompt when needed.
...
| `gatehouse_send_message` | Coordination messages (not for skill rollup registration) |
| `gatehouse_skill_summary_record` | Register `curator-summary.md` after skill rollup; Gatehouse auto-notifies {{lead_name}} when rollup is complete |
| `gatehouse_list_team` | No args: outer contacts + execution tree |
...
1. Read `.gatehouse/trees/<id>/reports/skills/<node_id>-extract.md`, `-verify.md`, and `.gatehouse/skills/by-domain/` changes.
...
4. **`gatehouse_skill_summary_record`** — required when skill domains were assigned (do not `send_message` {{lead_name}} for rollup).
...
| Avoid | Removing placeholder lines, splitting per-domain templates, editing during execution (rollup phase only) |
...
| Verify reports | `.gatehouse/trees/<id>/reports/skills/<node_id>-verify.md` |
